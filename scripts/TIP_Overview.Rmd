...
ggplot(data = d23_fl_means, aes(x= treatment, y = sugar_avg_mean, 
  ...
  scale_color_manual(values=four_colors)+
  labs(
    title="Mean Sugar Content by Inoculation Treatment in Tomato"
  ) +
...
predict_plot(mod23_phi2_2, d23_ps2_joined, phips2, daysfromgermination, treatment, 100,
  ...
  ylab("PhiPS2")+
  xlab("Days From Germination")+
  labs(title = str_wrap("Observed data (points) vs predicted (lines with 95% confidence interval) for photosystem II efficiency across days from germination and M. oryzae inoculation location in salt stressed tomato plants", 50))+
  guides(fill=guide_legend(title="Treatment"),
         color=guide_legend(title="Treatment"))+
  scale_color_scico_d(begin = 0.9, end = 0.1, palette = a_palette)+
  scale_fill_scico_d(begin = 0.9, end = 0.1, palette = a_palette)
...
predict_plot(mod23_gsw, d23_li, gsw, rh_s, treatment, 100,
  ...
  ylab("Stomatal Conductance  (mol m-2 s-1)")+
  xlab("Relative Humidity (%)")+
  labs(title = str_wrap("Observed data (points) vs predicted (lines with 95% confidence interval) for stomatal conductance across relative humidity and M. oryzae inoculation location in salt stressed tomato", 50))+
  ylim(0, 2.5)+
  guides(fill=guide_legend(title="Treatment"),
         color=guide_legend(title="Treatment"))+
  scale_color_scico_d(begin = 0.9, end = 0.1, palette = a_palette)+
...
predict_plot(mod24_gsw, filter(d24_li, gsw>0), gsw, rh_s, treatment,
  ...
  xlab("Relative Humidity (%)")+
  ylim(0,2)+
  labs(title = str_wrap("Observed data (points) vs predicted (lines with 95% confidence interval) for stomatal conductance across relative humidity and microbial consortium inoculation timing in tomato plants", 50))+
  guides(
    fill = guide_legend(title="Treatment"),
    color= guide_legend(title="Treatment"))+
  scale_color_scico_d(begin = 0.9, end = 0.1, palette = a_palette)+
...
predict_plot(mod24_ps2_2, d24_ps2_joined, phips2, daysfromgermination, treatment, 100, correction = "logit")+
  ...
  xlab("Days From Germination")+
  ylim(0.55, 0.8)+
  labs(title = str_wrap("Observed data (points) vs predicted (lines with 95% confidence interval) for photosystem II efficiency across days from germination and microbial consortium inoculation timing in tomato plants", 50))+
  guides(
    fill = guide_legend(title="Treatment"),
    color= guide_legend(title="Treatment"))+
  scale_color_scico_d(begin = 0.9, end = 0.1, palette = a_palette)+
