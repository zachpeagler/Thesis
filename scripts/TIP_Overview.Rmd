...
ggplot(data = d23_fl_means, aes(x= treatment, y = mass_mean, 
  ...
  scale_color_manual(values=four_colors)+
  labs(
    title= str_wrap("Mean Fruit Mass by Inoculation Treatment in Tomato Plants", 60)
  ) +
...
## sugar
mod23_sug <- lm(log(sugar_grams_mean) ~ treatment, data = d23_fl_means)
AIC(mod23_sug)
summary(mod23_sug)
...
ggplot(data = d23_fl_means, aes(x= treatment, y = sugar_grams_mean, 
  ...
  scale_color_manual(values=four_colors)+
  labs(
    title="Mean Sugar Mass by Inoculation Treatment in Tomato"
  ) +
    annotate("text", x=1:4, y=9.5, label = mod23_sug_letters, size=10, family="open")+
  ylab("Sugar (g)")+
  xlab("Treatment")+
  theme_bw()+
  theme(
...
## fruit count
mod23_fc <- glm(fruit_sum ~ treatment, data = d23_fl_summary,
...
ggplot(data = d23_fl_summary, aes(x= treatment, y = fruit_sum, 
  ...
  annotate("text", x=1:4, y=35, label = mod23_fc_letters, size=10, family="open")+
  ylab("Fruit")+
  xlab("Treatment")+
  theme_bw()+
  theme(
...
# fluorescence
## phi2
mod23_phi2 <- lmer(logit(phips2) ~ treatment + AmbientLight + (1 | Device), data = d23_ps2_joined)
AIC(mod23_phi2)
summary(mod23_phi2)
...
predict_plot(mod23_phi2_2, d23_ps2_joined, phips2, daysfromgermination, treatment, 100,
  ...
  ylab("PhiPS2")+
  xlab("Days From Germination")+
  labs(title = str_wrap("Real vs Predicted 95% confidence interval for photosystem II efficiency across days from germination and M. oryzae inoculation location in salt stressed tomato plants", 50))+
  guides(fill=guide_legend(title="Treatment"),
         color=guide_legend(title="Treatment"))+
  scale_color_scico_d(begin = 0.9, end = 0.1, palette = a_palette)+
  scale_fill_scico_d(begin = 0.9, end = 0.1, palette = a_palette)
...
predict_plot(mod23_gsw, d23_li, gsw, rh_s, treatment, 100,
  ...
  ylab("Stomatal Conductance  (mol m-2 s-1)")+
  xlab("Relative Humidity (%)")+
  labs(title = str_wrap("Real vs Predicted 95% confidence interval for stomatal conductance across relative humidity and M. oryzae inoculation location in salt stressed tomato", 50))+
  ylim(0, 2.5)+
  guides(fill=guide_legend(title="Treatment"),
         color=guide_legend(title="Treatment"))+
  scale_color_scico_d(begin = 0.9, end = 0.1, palette = a_palette)+
...
predict_plot(mod24_gsw, filter(d24_li, gsw>0), gsw, rh_s, treatment,
  ...
  xlab("Relative Humidity (%)")+
  ylim(0,2)+
  labs(title = str_wrap("Real vs Predicted 95% confidence interval for stomatal conductance across relative humidity and microbial consortium inoculation timing in tomato plants", 50))+
  guides(
    fill = guide_legend(title="Treatment"),
    color= guide_legend(title="Treatment"))+
  scale_color_scico_d(begin = 0.9, end = 0.1, palette = a_palette)+
...
## phips2
mod24_ps2 <- lmer(logit(phips2) ~ treatment + AmbientLight + (1 | Device),
                       data = d24_ps2_joined)
summary(mod24_ps2)
AIC(mod24_ps2)
...
predict_plot(mod24_ps2_2, d24_ps2_joined, phips2, daysfromgermination, treatment, 100, correction = "logit")+
  ...
  xlab("Days From Germination")+
  ylim(0.55, 0.8)+
  labs(title = str_wrap("Real vs Predicted 95% confidence interval for photosystem II efficiency across days from germination and microbial consortium inoculation timing in tomato plants", 50))+
  guides(
    fill = guide_legend(title="Treatment"),
    color= guide_legend(title="Treatment"))+
  scale_color_scico_d(begin = 0.9, end = 0.1, palette = a_palette)+
